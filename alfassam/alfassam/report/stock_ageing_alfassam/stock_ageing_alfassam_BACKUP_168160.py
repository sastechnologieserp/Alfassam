...
def format_report_data(filters: Filters, item_details: Dict, to_date: str) -> List[Dict]:
	...
	for item, item_dict in item_details.items():
		...
		total_valuation1 = flt(range1 * item_dict["details"]["valuation_rate"], precision)
		total_valuation2 = flt(range2 * item_dict["details"]["valuation_rate"], precision)
		total_valuation3 = flt(range3 * item_dict["details"]["valuation_rate"], precision)
		total_valuation4 = flt(range4 * item_dict["details"]["valuation_rate"], precision)
		total_valuation5 = flt(range5 * item_dict["details"]["valuation_rate"], precision)
		total_valuation6 = flt(above_range5 * item_dict["details"]["valuation_rate"], precision)

		row = [details.name, details.item_name, details.description, details.item_group, details.valuation_rate]
		...
		row.append(total_valuation1)
		row.append(total_valuation2)
		row.append(total_valuation3)
		row.append(total_valuation4)
		row.append(total_valuation5)
		row.append(total_valuation6)
		data.append(row)
...
def get_range_age(filters: Filters, fifo_queue: List, to_date: str, item_dict: Dict) -> Tuple:
	...
	for item in fifo_queue:
		...
		elif age <= flt(filters.range4):
			range4 = flt(range4 + qty, precision)
		elif age <= flt(filters.range5):
			range5 = flt(range5 + qty, precision)
		else:
			above_range5 = flt(above_range5 + qty, precision)
...
def get_columns(filters: Filters) -> List[Dict]:
	...
	columns.extend(
		[
			...
			{"label": _("Total Valuation Range 1"), "fieldname": "total_valuation1", "fieldtype": "Data", "width": 100},
			{"label": _("Total Valuation Range 2"), "fieldname": "total_valuation2", "fieldtype": "Data", "width": 100},
			{"label": _("Total Valuation Range 3"), "fieldname": "total_valuation3", "fieldtype": "Data", "width": 100},
			{"label": _("Total Valuation Range 4"), "fieldname": "total_valuation4", "fieldtype": "Data", "width": 100},
			{"label": _("Total Valuation Range 5"), "fieldname": "total_valuation5", "fieldtype": "Data", "width": 100},
			{"label": _("Total Valuation Range 6"), "fieldname": "total_valuation6", "fieldtype": "Data", "width": 100},
		]
	)
...
def setup_ageing_columns(filters: Filters, range_columns: List):
	ranges = [
		...
		f"{cint(filters['range4']) + 1} - {cint(filters['range5'])}",
		_("{0} - Above").format(cint(filters["range5"]) + 1),
	]
...
>>>>>>> 1a50e7263179cc5fae94bd57f57d66bda494d501
